...
def objectives(point):
    print('objectives_point=',point)
    #e.g., format:  {'x': 0.2, 'nx': 20, 'ny': 20, 'nz': 20, 'nodes': 1, 'cores': 4}
######################################### 
...
def main(): 
    ...
    plot=1
    import csv
    csv_file = 'hypre2d_dimmerge_grid_data_push.csv'
    ...
    if plot==1:
        ...
        for tid in range(len(giventask)):
            ...
            for i in range(len(x)):
                ...
                with open('hypre2d_dimmerge_grid_data_push.csv', 'a') as f:
                    f.write(f"{x[i]},{y[i][0]}\n")
            fontsize=40
            ...
            fig.savefig('obj2ddimmerge_nx_%f.pdf'%t)
            import pandas as pd

            # Assuming 'x' and 'y' are defined as in your plotting code
            # x = np.arange(0., 3.9999, 0.001)
            # y = calculated as per your objectives function

            # Combine x and y into a two-dimensional array
            xy_data = np.hstack((x.reshape(-1, 1), y))

            # Create a DataFrame
            df = pd.DataFrame(xy_data, columns=['x', 'y'])

            # Save the DataFrame to a CSV file
            df.to_csv('hypre2d_dimmerge_grid_data.csv', index=False)
...
if __name__ == "__main__":
    ...
    val = objectives({'x': 0.4, 'nx': 20, 'ny': 20, 'nz': 20, 'nodes': 1, 'cores': 4})
    print('done',val)
    main()
