...
# Load the data
df = pd.read_csv('/home/hrluo/gptune_cgp/GPTune/examples/Hypre/hypre2d_grid_data_push.csv')

# Prepare the data for interpolation
points = df[['trunc_factor','coarsen_type']].values  # The grid points where we have data
values = df['result'].values  # The corresponding time values
...
def f_truth(X):
    ...
    X = np.array(X).reshape(1,-1) # Ensure X is a 1D array

    # Calculate the Euclidean distance between X and each point in the points array
    distances = np.linalg.norm(points - X)
...
def get_bounds(restrict):
    #if restrict == 1:
    bds = np.array([[0,1],[0,8]]).astype(float).reshape(-1,2)
    return bds
