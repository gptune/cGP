...
def main(): 
    ...
    # Define the range for trunc_factor and coarsen_type
    trunc_factors = np.arange(0., 1.0, 1e-4)  # Example range for trunc_factor
    coarsen_types = [0,1,6,8]  # Example range for coarsen_type, assuming it's an integer

    # Prepare a list to store the results
    results = []
    import csv
    # Define the CSV file and write the header
    csv_file = 'hypre2d_grid_data_push.csv'
    with open(csv_file, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['trunc_factor', 'coarsen_type', 'result'])
        
    # Double loop over trunc_factor and coarsen_type
    for trunc_factor in trunc_factors:
        for coarsen_type in coarsen_types:
            # Call the objectives function with the current parameters
            result = objectives({'x': 0.4, 'nx': 20, 'ny': 20, 'nz': 20, 'nodes': 1, 'cores': 4, 
                                 'coarsen_type': coarsen_type, 'trunc_factor': trunc_factor})
            # Append the result along with the current parameters to the results list
            results.append([trunc_factor, coarsen_type, result])
            # Write the current result to the CSV file
            #writer.writerow([trunc_factor, coarsen_type, result])
            with open('hypre2d_grid_data_push.csv', 'a') as f:
                    f.write(f"{trunc_factor},{coarsen_type},{result[0]}\n")
    # Convert the results to a DataFrame
    df_results = pd.DataFrame(results, columns=['trunc_factor', 'coarsen_type', 'result'])

    # Save the DataFrame to a CSV file
    df_results.to_csv('hypre2d_grid_data.csv', index=False)
...
if __name__ == "__main__":
    ...
    val = objectives({'x': 0.4, 'nx': 20, 'ny': 20, 'nz': 20, 'nodes': 1, 'cores': 4, 'coarsen_type': 1, 'trunc_factor': 1.2})
    print('done',val)
    main()
